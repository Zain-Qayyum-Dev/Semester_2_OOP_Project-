...
public class Enemy {
    ...
    private int radialFireTimer = 0;
    private int fireCycleState = 0; // 0 for 5s, 1 for 3s
    ...
    private void loadSprite() {
        ...
        try {
            ...
            if (type == Type.BOSS_VOLCANO) {
                try {
                    BufferedImage fireRaw = ImageIO.read(getClass().getResourceAsStream("Boss_Volcano_MouthOpen.png"));
                    if (fireRaw != null) fireSprite = removeCheckerboard(fireRaw);
    ...
    /**
     * Removes green-screen background from sprites.
     * Only green-like pixels are made transparent.
     */
    private static BufferedImage removeCheckerboard(BufferedImage src) {
        ...
        for (int py = 0; py < h; py++) {
            for (int px = 0; px < w; px++) {
                ...
                // Green-screen key: remove pixels where green strongly dominates.
                boolean isGreenScreen = (g >= 110) && (g - r >= 35) && (g - b >= 35);
    ...
    private static String spriteFile(Type t) {
        switch (t) {
            ...
            case BOSS_DESERT:     return "Boss_Desert.png";
    ...
    public void update() {
        ...
        if (fireAnimTimer > 0) fireAnimTimer--;
    ...
    private void fireRadial() {
        ...
        Color bulletColor = (type.name().contains("DESERT")) ? new Color(255, 200, 100) : new Color(100, 255, 100);
        for (int i = 0; i < projectileCount; i++) {
            double angle = (Math.PI * 2 * i) / projectileCount;
            double vx = Math.cos(angle) * shotSpeed;
            double vy = Math.sin(angle) * shotSpeed;
            projectiles.add(new EnemyProjectile((int)cx, (int)cy, vx, vy, screenHeight, bulletColor));
        }
    }
    ...
    public void triggerVolcanoBossFireAnim() {
        if (type == Type.BOSS_VOLCANO && fireSprite != null) fireAnimTimer = 10;
    }
    ...
    public void draw(Graphics g) {
        ...
        BufferedImage frame = (type == Type.BOSS_VOLCANO && fireAnimTimer > 0 && fireSprite != null)
                            ? fireSprite : sprite;
        if (frame != null) {
            ...
            // Some sprites (like FOREST_1_PATROL) naturally face LEFT in their PNG files.
            // We flip the logic for them so they always face the way they move.
            boolean facingRight = naturallyFacesRight() ? movingRight : !movingRight;

            if (facingRight) {
                g2.drawImage(frame, dx, dy, dw, dh, null);
            } else {
                g2.drawImage(frame, dx + dw, dy, -dw, dh, null);
            }
        } else {
    ...
    private boolean naturallyFacesRight() {
        // Based on user report, FOREST_1_PATROL faces left by default.
        // We can add other types here if they also face the wrong way.
        switch (type) {
            case FOREST_1_PATROL: return false; 
            default:              return true; 
        }
    }
